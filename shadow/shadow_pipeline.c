...
static bool colors_match(const uint8_t *pixel, const ShadowColor *color);
static void set_pixel(uint8_t *pixel, const ShadowColor *color);
...
int run_shadow(const ShadowOptions *options) {
  ...
  uint8_t *stop_map = NULL;
  uint32_t width = 0;
  uint32_t height = 0;

  if (options == NULL) {
    fprintf(stderr, "Missing shadow options.\n");
    return -1;
  }
  ...
  printf(
      "shadow: %s -> %s (%zu layers, from=%s)\n",
      options->input_dir,
      options->output_dir,
      layer_count,
      options->direction == SHADOW_FROM_TOP ? "top" : "bottom");

  for (size_t step = 0; step < layer_count; ++step) {
    const size_t i = options->direction == SHADOW_FROM_TOP ? (layer_count - 1u - step) : step;
    Image image;
    ...
    if (step == 0u) {
      width = image.width;
      height = image.height;
      stop_map = calloc((size_t) width * (size_t) height, sizeof(*stop_map));
      if (stop_map == NULL) {
        fprintf(stderr, "Failed to allocate shadow stop map.\n");
        free_image(&image);
        free_layer_files(layers, layer_count);
        return -1;
      }
    ...
    for (uint32_t y = 0; y < image.height; ++y) {
      for (uint32_t x = 0; x < image.width; ++x) {
        const size_t pixel_index = (size_t) y * (size_t) image.width + (size_t) x;
        uint8_t *pixel = image.pixels + (size_t) y * image.stride + (size_t) x * 4u;
        if (stop_map[pixel_index] != 0u) {
          continue;
        }

        if (colors_match(pixel, &options->replace_color)) {
          set_pixel(pixel, &options->set_color);
        } else {
          stop_map[pixel_index] = 1u;
        }
      }
    }
    ...
    printf("Layer %zu/%zu (%s)\n", step + 1u, layer_count, layers[i].name);
    fflush(stdout);
